...
const RTTI CE_Bomb_1::m_Rtti("CE_Bomb_1", CEnemyBullet::m_Rtti);

void CE_Bomb_1::isOutOfFocus()
{
...
void CE_Bomb_1::SetAngle(float _fAngle)
{
...
void CE_Bomb_1::CheckLineCollision()
{
...
void CE_Bomb_1::CheckCollision(CObj* _Something)
{
	if (_Something->GetRTTI().isExactly(CPlayer::m_Rtti))
		isDead = true;
}

void CE_Bomb_1::Initialize(void)
{
	...
	m_Ani = CAniInfoMgr::Getinst()->GetAniDCSet(_T("m_Enemy_Weapon_AniSet"), _T("EW_BOMB_1"));
	//m_Ani = CAniInfoMgr::Getinst()->GetEnemy_Weapon_AniSet(_T("EW_BOMB_1"));
	m_CurFrame = 0;
...
int CE_Bomb_1::Progress(void)
{
	...
	else
	{
		...
		((CExplosion*)Explosion)->SetExplosionName("Explosion_FireBomb");
		CObjMgr::GetInst()->GetObjList(OBJ_EFFECT)->push_back(Explosion);
		return 1;
	}
}
void CE_Bomb_1::Render(HDC _hdc)
{
