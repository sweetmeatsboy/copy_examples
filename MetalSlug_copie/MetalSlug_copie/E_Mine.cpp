...
const RTTI CE_Mine::m_Rtti("CE_Mine", CEnemyBullet::m_Rtti);

void CE_Mine::isOutOfFocus()
{
...
void CE_Mine::SetAngle(float _fAngle)
{
...
void CE_Mine::CheckLineCollision()
{
...
void CE_Mine::CheckCollision(CObj* _Something)
{
...
int CE_Mine::Progress(void)
{
	...
	{
		CObj* Explosion = CObjFactory<CExplosion>::CreateObj(m_CenterPoint.getX(), m_CenterPoint.getY());
		((CExplosion*)Explosion)->SetExplosionName("Explosion_Mine_2");
		CObjMgr::GetInst()->GetObjList(OBJ_EFFECT)->push_back(Explosion);
		return 1;
	}
}
void CE_Mine::Render(HDC _hdc)
{
