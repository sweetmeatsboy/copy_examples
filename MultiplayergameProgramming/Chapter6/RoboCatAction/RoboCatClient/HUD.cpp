...
{
	float rttMS = NetworkManagerClient::sInstance
		->GetAvgRoundTripTime().GetValue() * 1000.f;
...
void HUD::RenderText(const string& inStr, const Vector3& origin, const Vector3& inColor)
{
	// Convert the color
	SDL_Color color;
	color.r = static_cast<Uint8>(inColor.x * 255);
	color.g = static_cast<Uint8>(inColor.y * 255);
	color.b = static_cast<Uint8>(inColor.z * 255);
	color.a = 255;
